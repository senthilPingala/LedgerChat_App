...
import pingala.qa.ledgerchat.screens.OneToOneChatWindow.scrollDirection;
import pingala.qa.ledgerechat.contantutils.Constants;

public class OneToGroupChatScreen {
	...
	private By selectedSilentGroup = By.xpath("//android.widget.TextView[@text=\"Audit_Grp\"]");
	private By usrname = By.xpath("//android.widget.TextView[@text=\"vinayaka hk\"]");
	private By backButton = By.xpath("//android.widget.Button[@content-desc=\"back-icon\"]");
    ...
    private By readOnlyAndDeletedGroupInfoMsg = By.xpath("//android.widget.TextView[@text=\"You can't send message to this group\"]");
    private By activeGroupCheck = By.xpath("//android.widget.EditText[@text=\"Type a message..\"]");
    
    
	
	// 2. Page Constructor:
	public OneToGroupChatScreen(AppiumDriver driver) {
		this.driver = driver;
	...
	public String openReadOnlyGroupToChat(String groupName) {

		elemUtil.doClickFromList(chatListName, groupName);
		//elemUtil.waitForElementPresentAndClick(allowPermission, Constants.DEFAULT_TIME_OUT);
		String deletedGrpInfo = elemUtil.waitForElementGetText(readOnlyAndDeletedGroupInfoMsg, Constants.DEFAULT_TIME_OUT);
		elemUtil.doClick(backButton);
		return deletedGrpInfo;
	}
	
	public String openDeletedGroupToChat(String groupName) {

		elemUtil.doClickFromList(chatListName, groupName);
		elemUtil.waitForElementPresentAndClick(allowPermission, Constants.DEFAULT_TIME_OUT);
		String usrRemovedInfo = elemUtil.waitForElementGetText(readOnlyAndDeletedGroupInfoMsg, Constants.DEFAULT_TIME_OUT);
		elemUtil.doClick(backButton);
		return usrRemovedInfo;
	}
	
	public String openActiveGroupToChat(String groupName) {

		elemUtil.doClickFromList(chatListName, groupName);
		//elemUtil.waitForElementPresentAndClick(frontCamereAccessPerm, Constants.DEFAULT_TIME_OUT);
		String grpAllowedToMessage = elemUtil.waitForElementGetText(activeGroupCheck, Constants.DEFAULT_TIME_OUT);
		return grpAllowedToMessage;
	}
	...
	public String attachImgThroughGallery() {
		...
		String imageText =  elemUtil.waitForElementGetText(sentImageInChat, Constants.DEFAULT_TIME_OUT);
		return imageText;
	}
	
	public String attachImgForForward() throws InterruptedException {
		Thread.sleep(3000);
		elemUtil.waitForElementPresentAndClick(attachmentBtn, Constants.DEFAULT_TIME_OUT);
		elemUtil.waitForElementPresentAndClick(imageGalleryBtn, Constants.DEFAULT_TIME_OUT);
...
//		elemUtil.waitForElementPresentAndClick(allowPermission, Constants.DEFAULT_TIME_OUT);
	...
	public String forwardOptionPop() throws InterruptedException {
		Thread.sleep(3000);
